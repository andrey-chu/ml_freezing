...
This file implements HMM using seqlearn package HHM-routines
@author: andrey
...
def my_diff(param_to_diff):
    return np.gradient(param_to_diff)
fun_dict ={
        "lin":linear_f,
        "grad":my_diff
        }
...
with h5py.File(united_dataset, 'r') as f, h5py.File(add_dataset1, 'r') as f1:
    ...
    if change_labels_bool == 1:
        new_labels = change_labels_preserve(labels,2)
        new_labels_add = change_labels_preserve(labels_add,2)
        new_matlab = change_labels_preserve(matlab,2)
        new_matlab_add = change_labels_preserve(matlab_add,2)
    else:
