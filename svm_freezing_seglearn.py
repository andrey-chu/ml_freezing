...
with h5py.File(united_dataset, 'r') as f:
    ...
    clf = Pype([('segment', SegmentXY(width=chosenwidth,step=1)), # in this context what is the difference with SegmentX?
           ('features', FunctionTransformer(reshape_all)),
            ('scaler', StandardScaler()),
            ('bagg', OneVsRestClassifier(BaggingClassifier(SVC(kernel='rbf', gamma=gamma_chosen, C=C_chosen, probability=True, class_weight='balanced'), max_samples=1.0 / n_estimators, warm_start=True, n_estimators=n_estimators, n_jobs=6, verbose=10)))], scorer=scorer1)
    ...
    # Let's remove 2's because they are meaningless for finding freezing points
    test11_2 = test11
    test21_2 = test21
    Matlab_test_2 = Matlab_test
    for i in range(len(test21_2)):
        test21_2[i][test21[i]==2]=0
        test11_2[i][test11[i]==2]=0
        Matlab_test_2[i][Matlab_test[i]==2]=0
    
    
    # and do a cross validation
    Xc = np.column_stack((datasets_included, substance_included))
    Xt = np.array(new_features_seg_included)
    X = TS_Data(Xt, Xc)
    y = np.array(new_labels_seg_included)
    scoring = make_scorer(f1_score, average='macro')
    freeze2 = [np.argmax(test21_2[i]) for i in range(len(test21_2))]
    freeze2_conserve = [test21_2[i].shape[0]-np.argmax(np.flip(np.int_(np.logical_not(test21_2[i])))) for i in range(len(test21_2))]
    freeze1 = [np.argmax(test11_2[i]) for i in range(len(test11_2))]
    freeze1_conserve = [test11_2[i].shape[0]-np.argmax(np.flip(np.int_(np.logical_not(test11_2[i])))) for i in range(len(test11_2))]
    
    mat_freeze1 = [np.argmax(Matlab_test_2[i]) for i in range(len(Matlab_test_2))]
    mat_freeze1_conserve = [Matlab_test_2[i].shape[0]-np.argmax(np.flip(np.int_(np.logical_not(Matlab_test_2[i])))) for i in range(len(Matlab_test_2))]
    # cv_scores = cross_validate(clf, X, y, cv=4, return_train_score=True)
    (err12_conserve, mean_dist12_conserve, _)=supp_methods.freezing_metrics(np.asarray(freeze2_conserve),np.asarray(freeze1), 10)
    (err12, mean_dist12, _)=supp_methods.freezing_metrics(np.asarray(freeze2),np.asarray(freeze1), 10)
    (err12_mat, mean_dist12_mat, _)=supp_methods.freezing_metrics(np.asarray(mat_freeze1_conserve),np.asarray(freeze1), 10)
